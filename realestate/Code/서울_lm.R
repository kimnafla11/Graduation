...
for(i in 12:96){
  source("Code/Functions/DIFFMOVINGWINDOW_FUNCTION.r")
  lmw=lmw(diff_seoul,i)
  lmw = na.omit(lmw)
  dmwr[i,]= mean((diff_seoul[i:109,17]-lmw)^2)
}
dmwr = as.data.frame(dmwr)
plot(dmwr$V1,type='l',xlim=c(12,96), ylim=c(2500,5000))
min(dmwr[12:96,])
which(dmwr<=2681) #optimal ws 79 mse=2680.83
dmwr[79,]


source("Code/Functions/DIFFMOVINGWINDOW_FUNCTION.r")
lmw = lmw(diff_seoul,79)
plot(diff_seoul[,17],type='l')
points(lmw,type ='l',col='red')
abline(v=79, col='blue')

r = diff_seoul$V17[79:109]-lmw[79:110]
plot(r, type = 'o')
