...
########mw없이RF############
train = diff_seoul[1:80,]
test = diff_seoul[-(1:80),]
nrow(test)
...
for(i in 12:96){
  source("Code/Functions/DIFFRANDOMFOREST_FUNCTION.r")
  rmw=rmw(diff_seoul,i)
  rmw = na.omit(rmw)
  dmwr[i,]= mean((diff_seoul[i:109,17]-rmw)^2)
}
plot(dmwr$V1,type='l',xlim=c(12,96), ylim=c(1000,4000))
min(dmwr[12:96,])
which(dmwr<=1320) #optimal ws 13 mse=1319.442

source("Code/Functions/DIFFRANDOMFOREST_FUNCTION.r")
rmw = rmw(diff_seoul,13)
plot(diff_seoul[,17],type='l')
points(rmw,type ='l',col='red')
abline(v=13, col='blue')


r = diff_seoul$V17[13:109]-rmw[13:110]
plot(r, type = 'o')
