...
mw = function(stat, w.s){
  ...
  while(tails<= nrow(stat)){
    ...
    window.gbm = gbm(window.stat$평균아파트매매가격.강원.~.,data = window.stat)
...
for(i in 43:96){
  gmw=mw(house,i)
  dmwr[i,]= mean((house[i:104,17]-gmw[i:105])^2)
}
plot(dmwr$V1,type='l',xlim=c(43,96))
min(dmwr[43:96,])
which(dmwr<=583) #optimal ws 43 mse=582.4732
...
res = house[44:104,17]-gmw[44:104]
plot(res, type='o')
