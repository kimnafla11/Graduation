...
mw = function(stat, w.s){
  ...
  while(tails<= nrow(stat)){
    ...
    window.rf = randomForest(window.stat$평균아파트매매가격.전국.~., data = window.stat)
    #randomForest
...
for(i in 12:96){
  rmw=mw(house,i)
  rmw = na.omit(rmw)
  dmwr[i,]= mean((house[i:109,17]-rmw)^2)
}
plot(dmwr$V1,type='l',xlim=c(12,96), ylim=c(2500,160000))
min(dmwr[12:96,])
which(dmwr<=14657) #optimal ws 12 mse=14656.97
...
r = house$평균아파트매매가격.전국.[13:109]-rmw[13:109]
plot(r, type = 'o')
