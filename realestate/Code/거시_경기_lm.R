...
mw = function(stat, w.s){
  ...
  while(tails<= nrow(stat)){
    ...
    window.lm = lm(window.stat$평균아파트매매가격.경기.~.,data = window.stat)
    #linear regression
...
for(i in 12:96){
  lmw=mw(house,i)
  lmw = na.omit(lmw)
  dmwr[i,]= mean((house[i:104,17]-lmw)^2)
}
dmwr = as.data.frame(dmwr)
plot(dmwr$V1,type='l',xlim=c(12,96),ylim=c(40000,500000))
min(dmwr[12:96,])
which(dmwr<=46192) #optimal ws 28 mse=46191.83
...
r = house$평균아파트매매가격.경기.[29:104]-lmw[29:104]
plot(r, type = 'o')
