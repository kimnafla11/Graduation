house = read.csv('Data/kang11lag.csv')
house = house[,c(-1,-2)]
#######
...
mw = function(stat, w.s){
  ...
  while(tails<= nrow(stat)){
    ...
    window.lm = lm(window.stat$평균아파트매매가격.강원.~.,data = window.stat)
    #linear regression
...
min(dmwr[12:96,])
which(dmwr<=6450) #optimal ws 22 mse=6449.543
...
r = house$평균아파트매매가격.강원.[20:104]-lmw[20:104]
plot(r, type = 'o')
